...
public:
  ...
  /** Get the type of MI metric. */
  itkGetMacro( MetricType, unsigned short );
  ...
  //add by pax
  itkSetStringMacro( MovingImageName );
  itkSetStringMacro( FixedImageName );
  itkSetStringMacro( OutputImageName );

  void get_all(const fs::path& root, const std::string& ext, std::vector<std::string>& imageFileNames);
  static bool compare_by_slice(const std::string& fn1, const std::string& fn2);
...
private:
  ...
  //pax
  std::string			m_FixedImageName;
  std::string			m_MovingImageName;

...
def filter_volume(np_tensor_3d, size_patch, trainfile,local_mean_std=False,
                  extramargin=None, model=None, speedup=None):


    np_tensor_3d = np_tensor_3d / 255.0
    ...
    data = np.zeros((len(rangez) * n_points, patchlen))
    iter_z=0
    for z0 in bar_prediction(rangez):
        i = 0
        for y0 in rangey:
            for x0 in rangex:
                patch = np_tensor_3d[z0-size_patch:z0+size_patch+1,
                                     y0-size_patch:y0+size_patch+1,
                                     x0-size_patch:x0+size_patch+1]
                data[iter_z * n_points + i] = np.reshape(patch, (patchlen,))
                i += 1
        iter_z += 1
    ...
    data = predictor.predict(data, model, n_views=1)

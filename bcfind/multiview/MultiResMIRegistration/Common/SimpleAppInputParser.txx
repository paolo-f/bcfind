...
::get_all(const fs::path& root, const std::string& ext, std::vector<std::string>& imageFileNames)
{  
  ...
  if (fs::is_directory(root))
  {
    ...
    while(it != endit)
    {
      ...
      {
	imageFileNames.push_back(it->path().string());
      }
      ++it;
    }
  }
...
bool
SimpleAppInputParser<TImage>
::compare_by_slice(const std::string& fn1, const std::string& fn2) {
    
	std::string name_1 = fn1.substr(fn1.find_last_of("/\\")+1);
        std::string name_2 = fn2.substr(fn2.find_last_of("/\\")+1);
        name_1 = name_1.substr(0, name_1.find_last_of("."));
        name_2 = name_2.substr(0, name_2.find_last_of("."));
...
::Execute()
{
  ...
  if( fscanf( paramFile, "%f", &fNumber) != 1 )
    {
    itkExceptionMacro( << "Could not find the number of spatial samples." );
    }
  m_NumberOfSpatialSamples = fNumber;
  std::cout << "Number of spatial samples: " << fNumber << std::endl;


 if( fscanf( paramFile, "%f", &fNumber) != 1 )  //ViolaMI(0)  MattesMI(1)
    {
    itkExceptionMacro( << "Could not find the type of Metric [ViolaMI or MattesMI]." );
    }
  m_MetricType = fNumber;
  if (fNumber == 1)
    std::cout << "Mutual Information MetricType:  Mattes" << std::endl;
  else if  (fNumber == 0)
    std::cout << "Mutual Information MetricType:  Viola" << std::endl;
  else if  (fNumber == 2)
    std::cout << "Mutual Information MetricType:  Normalized Mattes" << std::endl;
  else{
    std::cout<<"Error!!! The selected MetricType doesn't exist  (es. 0 for ViolaMI, 1 for MattesMI, 2 for MattesNMI)\n";
    std::exit(1);
    }
   ...
   ***********************/
   ...
   /***********************
   * Read in the images add py pax
   ***********************/ 

  fs::path target_folder(m_FixedImageName);
  std::string ext = ".tif";
  std::vector<std::string> targetFileNames;
  SimpleAppInputParser::get_all(target_folder, ext, targetFileNames);
  std::sort(targetFileNames.begin(), targetFileNames.end(), SimpleAppInputParser::compare_by_slice); 
  std::cout << "Fixed image filename folder: " << m_FixedImageName << std::endl;


  fs::path input_folder(m_MovingImageName);
  std::vector<std::string> inputFileNames;
  get_all(input_folder, ext, inputFileNames);
  std::sort(inputFileNames.begin(), inputFileNames.end(), compare_by_slice);
  std::cout << "Moving image filename folder: " << m_MovingImageName << std::endl;

  /* for(std::vector<std::string>::iterator it = inputFileNames.begin(); it != inputFileNames.end(); ++it){
    std::cout << *it<<"\n";	
   }
   
   for(std::vector<std::string>::iterator it = targetFileNames.begin(); it != targetFileNames.end(); ++it){
    std::cout << *it<<"\n";	
   } */
   ...
   try 
    {
    fixedReader->SetImageIO( TIFFImageIO::New() );
    fixedReader->SetFileNames(targetFileNames );
    fixedReader->Update();
   ...
   try 
    {
    movingReader->SetImageIO( TIFFImageIO::New() );
    movingReader->SetFileNames(inputFileNames);
    movingReader->Update();
   ...
   /************************************
   * Parse output related parameters by pax
   ************************************/

  m_OutputFileName = m_OutputImageName;

  std::cout << "Output image filename folder: " << m_OutputFileName << std::endl;
  std::cout << std::endl;
